...
void init_hash_set(HashSet* set, int initial_capacity){ // инициализация таблицы
...
int hash_function(const HashSet* set, int value){ // вычисление хеш-кода
...
bool contains(const HashSet* set, int value){ // проверка наличия элемента
...
bool insert_value(HashSet* set, int value){ // вставка элемента в начало цепочки
...
bool delete_value(HashSet* set, int value){ // удаление элемента
    int index = hash_function(set, value);
    Node* current = set -> buckets[index];
    Node* prev = nullptr;
...
void clear(HashSet* set){ // очистка
...
void destroy_hash_set(HashSet* set) { // освобождение памяти
    clear(set);
...
void parse_arguments(int argc, char* argv[], string& filename, string& query) { // разбор аргументов командной строки
...
void load_data_from_file(HashSet* set, const string& filename) { // загрузка данных из файла
...
void save_data_to_file(const HashSet* set, const string& filename) { // сохранение данных в файл
...
void process_query(HashSet* set, const string& query) { // обработка запроса
