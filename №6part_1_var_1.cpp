...
public:
    ...
    void insert(int key, const string& value) {//вставка элемента
    ...
    bool remove(int key) { // Удаление эелмента
        int index = hashFunction(key);
        Node* current = table[index];
        Node* prev = nullptr;
    ...
    int getCount() const {// возвращает количество элементов
        return count;
    }

    double getLoadFactor() const {//вычисляет отношение элементов к размеру
...
public:
    ...
    void insert(int key, const string& value) {//проверяет коэффициент загрузки
        if (count >= size * 0.75) {
